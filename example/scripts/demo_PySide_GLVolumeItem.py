...
np.clip(
    data, a_min=0, a_max=0.25 * data.max(), out=data
)  # clip data between 0% and 25% of maximum
data = (nPoints * data / data.max()).astype(
    np.ubyte
)  # convert data to byte-array with values ranging from 0 to 255
