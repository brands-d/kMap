...
class Plot3DMolecule:
    def __init__(self, w, orbital, grid=True, photon={}, isoval=1 / 6):
        ...
        for bond in bonds:
            line = gl.GLLinePlotItem(
                pos=bond, color=(0.8, 0.8, 0.8, 1), width=5, antialias=True
            )
            items.append(line)
            w.addItem(line)
        ...
        if "polarization" in photon:  # display incident photon
            ...
            photon_line = gl.GLLinePlotItem(
                pos=pos, color=(1, 1, 0.0, 1), width=5, antialias=True
            )
    ...
    def rotate_molecule(
        self, phi=0, theta=0, psi=0
    ):  # Rotate using Euler angles phi, theta, psi
        old_phi = self.phi
        ...
        for item in self.items:
            item.rotate(
                old_psi, axes[2][0], axes[2][1], axes[2][2], local=True
            )  # first undo psi-rotation
            item.rotate(
                old_theta, axes[1][0], axes[1][1], axes[1][2], local=True
            )  # next undo theta-rotation
            item.rotate(
                old_phi, axes[0][0], axes[0][1], axes[0][2], local=True
            )  # finally undo phi-rotation

        # now rotate items to new desired orientation
        axes = get_rotation_axes(phi, theta)
        for item in self.items:
            item.rotate(
                -phi, axes[0][0], axes[0][1], axes[0][2], local=True
            )  # first do phi-rotation
            item.rotate(
                -theta, axes[1][0], axes[1][1], axes[1][2], local=True
            )  # next do theta-rotation
            item.rotate(
                -psi, axes[2][0], axes[2][1], axes[2][2], local=True
            )  # finally do psi-rotation
...
molecule_view = Plot3DMolecule(
    w, molecule, photon={"polarization": "p", "alpha": 60, "beta": 115}
)
