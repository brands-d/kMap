...
from kmap.library.misc import compute_Euler_matrix, energy_to_k
...
class Orbital:
    ...
    def __init__(
        ...
        if file_format == "cube":  # Read orbital data from cube-file
            ...
            self._read_local_hdf5(file, orbital_name)
    ...
    @classmethod
    def init_from_local_hdf5(
        cls, h5file, orbital_name, dk3D=0.15, E_kin_max=150, value="abs2"
    ):
        cls._read_local_hdf5(cls, h5file, orbital_name)
        self = cls("", file_format="hdf5", dk3D=dk3D, E_kin_max=E_kin_max, value=value)

        return self

    @classmethod
    def init_from_remote_hdf5(
        cls,
        server,
        port,
        moleculeID,
    ...
    def get_kmap(
        ...
        if new_cut:
            self.set_kinetic_energy(E_kin, dk, V0)
    ...
    def compute_3DFT(self, dk3D, E_kin_max, value):
        ...
        pad_x = max(int(2 * np.pi / (dk3D * self.psi["dx"])) - self.psi["nx"], 0)
        pad_y = max(int(2 * np.pi / (dk3D * self.psi["dy"])) - self.psi["ny"], 0)
        pad_z = max(int(2 * np.pi / (dk3D * self.psi["dz"])) - self.psi["nz"], 0)

        if pad_x % 2 == 1:
            pad_x += 1  # make sure it's an even number
        if pad_y % 2 == 1:
            pad_y += 1  # make sure it's an even number
        if pad_z % 2 == 1:
            pad_z += 1  # make sure it's an even number
        ...
        # computation
        psik_interp = interp.RegularGridInterpolator(
            (kx, ky, kz), psik, bounds_error=False, fill_value=np.nan
        )
    ...
    def check_new_cut(self, E_kin, dk, V0):
        ...
        if "E_kin" in self.kmap:
            if type(dk) != tuple and type(self.kmap["dk"]) != tuple:
                if (
                    np.abs(self.kmap["E_kin"] - E_kin) > eps
                    or np.abs(self.kmap["dk"] - dk) > eps
                ):
                    new_cut = True
    ...
    def set_orientation(self, phi, theta, psi):
        ...
        kxkykz = list(
            map(lambda a, b, c: (a, b, c), KXr.flatten(), KYr.flatten(), KZr.flatten())
        )
    ...
    def check_new_orientation(self, phi, theta, psi):
        ...
        if "phi" in self.kmap:
            if (
                np.abs(self.kmap["phi"] - phi) > eps
                or np.abs(self.kmap["theta"] - theta) > eps
                or np.abs(self.kmap["psi"] - psi) > eps
            ):
    ...
    def set_polarization(self, Ak_type, polarization, alpha, beta, gamma, s_share):
        ...
        # circularly polarized light can be simulated
            ...
            # Circularly polarized light (right-handed)
                ...
                Ak = (
                    0.5 * (polp**2 + gamma_calc**2 * sin_a**2)
                    + 0.5 * pols**2
                    + (sin_b * kx - cos_b * ky) * gamma_calc * sin_a
                )
    ...
    def plot(
        ...
        # plot kmap
        im = ax.imshow(
            data, extent=limits, interpolation=interpolation, origin="lower", cmap="jet"
        )
    ...
    def _read_cube(self, file):
        ...
        x0, y0, z0 = (
            b2a * float(lines[2].split()[1]),
            b2a * float(lines[2].split()[2]),
            b2a * float(lines[2].split()[3]),
        )
        nx, ny, nz = (
            int(lines[3].split()[0]),
            int(lines[4].split()[0]),
        ...
        dx, dy, dz = (
            b2a * float(lines[3].split()[1]),
            b2a * float(lines[4].split()[2]),
            b2a * float(lines[5].split()[3]),
        )
    ...
    def _read_local_hdf5(self, file, orbital_name):
        """Read orbital data from local hdf5 file."""

        if type(file) == str:
            with h5py.File("aux.hdf5", "r") as f:
                num_atom = f["num_atom"][()]
                chemical_numbers = f["chemical_numbers"][()]
                atomic_coordinates = f["atomic_coordinates"][()]
                x = f[orbital_name]["x"][()]
                y = f[orbital_name]["y"][()]
                z = f[orbital_name]["z"][()]
                data = f[orbital_name]["data"][()]

        else:
            num_atom = file["num_atom"][()]
            chemical_numbers = file["chemical_numbers"][()]
            atomic_coordinates = file["atomic_coordinates"][()]
            x = file["x"][()]
            y = file["y"][()]
            z = file["z"][()]
            data = file["data"][()]
