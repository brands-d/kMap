...
class Orbital():
    ...
    def _write_cube(self, filename):
        ...
        ny = self.psi['ny']
        nz = self.psi['nz']
        num_atom = self.molecule['num_atom']
        ...
        with open(filename, 'w') as file:
            ...
            for i in range(num_atom):
                Z = self.molecule['chemical_numbers'][i]
                x = self.molecule['atomic_coordinates'][i][0]/b2a
                y = self.molecule['atomic_coordinates'][i][1]/b2a
                z = self.molecule['atomic_coordinates'][i][2]/b2a
                print('%5i %10.6f  %10.6f  %10.6f  %10.6f'%(Z, Z, x, y, z), file=file)    
        
            # now write out the volume data
            for i in range(nx):
                for j in range(ny):
                    file.write(form%tuple(self.psi['data'][i,j,:]))
